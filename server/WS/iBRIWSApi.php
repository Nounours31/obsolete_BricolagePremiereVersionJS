...
abstract class iBRIWSApi {
    ...
    // ctor
    // --------------------------------------------------------------------------------------
    function __construct($className = "abstrart class iBRIWSApi") {
    ...
    function decodeInput() {
        ...
        if ($err -> FAILED()) {
            $this -> logger-> debug("buildFromArray:: failed - Contenu invalide - isArray :".is_array($contenu)." Contenu: -->".(is_array($contenu) ? BRITools::arrayToString($contenu) : $contenu));
            throw new Exception('unable to parse -- err:' . $err -> toString());
        }
        return $retour;
    }
    ...
    private function _decodeJSONInput() {
        ...
        if ($decoded == null) {
            ...
            throw new Exception('unable to parse -- invalid json format test it first ...');
        }
        else 
